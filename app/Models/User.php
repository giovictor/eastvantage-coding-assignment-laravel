    ...
    public static function getUsersWithRoles()
    {
        return self::leftJoin('role_user', 'users.id', 'role_user.user_id')
            ...
            ->select([
                ...
                DB::raw("(SELECT GROUP_CONCAT(roles.id ORDER BY roles.id) FROM roles JOIN role_user ON roles.id = role_user.role_id WHERE role_user.user_id = users.id) AS user_roles_id")
            ])
